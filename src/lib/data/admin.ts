...
import { mapArticleDoc, mapUserDoc } from "@/lib/data/mappers";
...
export async function getDashboardStats(): Promise<DashboardStats> {
  ...
  const topArticles = topSnap.docs.map((d) => mapArticleDoc(d.id, d.data()));
  const recentUsers = usersSnap.docs.map((d) => mapUserDoc(d.id, d.data()));

...
import { mapArticleDoc } from "@/lib/data/mappers";
...
export async function getAdminArticleById(id: string): Promise<Article | null> {
  ...
  return mapArticleDoc(snap.id, snap.data());
}
...
// Article.publishedAt / scheduledAt endi ISO-string (yoki null) sifatida saqlanadi (DateLike).
// Firestore'ga qayta yozishda ularni Timestamp'ga aylantirish shart, aks holda "publishedAt"
// maydoni ba'zi hujjatlarda Timestamp, ba'zilarida oddiy string bo'lib qolib, orderBy("publishedAt")
// so'rovlarini buzadi.
function isoToTimestamp(value: string | null | undefined): Timestamp | null {
...
export async function updateArticle(
  ...
  await updateDoc(doc(db, ARTICLES_COLLECTION, id), {
    ...
    status,
    featured: input.featured,
    publishedAt: publishedAt ?? isoToTimestamp(current?.publishedAt as string | null) ?? null,
    scheduledAt,
    updatedAt: serverTimestamp(),
    readingTimeMinutes: calculateReadingTime(input.content),
    seoTitle: input.seoTitle || "",

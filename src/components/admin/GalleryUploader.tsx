...
import { UploadCloud, X, Loader2, Link2, Plus } from "lucide-react";
...
export function GalleryUploader({ images, onChange }: GalleryUploaderProps) {
  ...
  const handleAddUrl = () => {
    ...
    onChange([...images, { url: trimmed, path: `external:${nanoid(8)}`, caption: "" }]);
    setUrlInput("");
    setUrlMode(false);
  };
  ...
  return (
    <div>
      <div className="mb-1.5 flex items-center justify-between">
        <label className="block text-xs font-medium text-muted">Galereya</label>
        <button
          type="button"
          onClick={() => setUrlMode((prev) => !prev)}
          className="glass-chip flex items-center gap-1 rounded-full px-2.5 py-1 text-[11px] font-medium text-muted transition hover:text-accent"
        >
          <Link2 size={12} /> Havola orqali qo'shish
        </button>
      </div>
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
        {images.map((image) => (
          <div key={image.path} className="space-y-1">
            <div className="relative aspect-square overflow-hidden rounded-lg border border-line dark:border-line-dark">
              <Image
                src={image.url}
                alt={image.caption || "Galereya rasmi"}
                fill
                unoptimized
        ...
        <label className="flex aspect-square cursor-pointer flex-col items-center justify-center rounded-lg border-2 border-dashed border-line text-muted transition hover:border-accent hover:text-accent dark:border-line-dark">
          ...
          <span className="mt-1 text-[11px]">Fayl yuklash</span>
          <input
            type="file"
            accept="image/*"
            multiple

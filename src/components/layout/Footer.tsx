...
export async function Footer() {
  ...
  return (
    <footer className="cinematic-gradient mt-16 border-t border-line bg-paper-dim dark:border-line-dark dark:bg-ink">
      <div className="container-page grid gap-10 py-12 md:grid-cols-4">
        <div className="md:col-span-2">
          <Link href="/" className="flex items-center gap-2">
            <Image src="/icons/logo.png" alt="FilmNews.uz" width={36} height={29} className="h-8 w-auto" />
            ...
            <div className="mt-5 flex gap-3">
              {socialLinks.map(({ key, href, icon: Icon, label }) => (
                <a
                  ...
                  aria-label={label}
                  className="glass-chip flex h-9 w-9 items-center justify-center rounded-full text-ink transition hover:border-accent hover:text-accent dark:text-paper"
                >
                  <Icon size={16} />
                </a>
              ))}
